...
public:
	BillboardRR(WCHAR* billb, WCHAR* normal, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, float escala)
	{
		...
		CargaParametros(billb,normal, escal, NULL);
	}

	BillboardRR(WCHAR* billb, WCHAR* normal, WCHAR* displacement, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, float escala)
	{
		//copiamos el device y el device context a la clase terreno
		d3dContext = D3DContext;
		d3dDevice = D3DDevice;
		...
		frontal = D3DXVECTOR3(0, 0, 1);
		//aqui cargamos las texturas de alturas y el cesped
		CargaParametros(billb, normal, escal, displacement);
	}
	...
	bool CargaParametros(WCHAR* billb, WCHAR* normal, float escala, WCHAR* displacement)
	{
		HRESULT d3dResult;
		ID3DBlob* vsBuffer = 0;
		bool compileResult;
		...
		if (!displacement) //TODO checa el agua
			compileResult = CompileD3DShader(L"Billboard.hlsl", "VS_Main", "vs_4_0", &vsBuffer);
		else
			compileResult = CompileD3DShader(L"water.hlsl", "VS_Main", "vs_4_0", &vsBuffer);
		...
		if (!displacement) //TODO checa el agua
			...
			compileResult = CompileD3DShader(L"water.hlsl", "PS_Main", "ps_4_0", &psBuffer);
		...
		if (displacement) {
			d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, displacement, 0, 0, &displacementMap, 0);
		}
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &MovimientoWater);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam, float xx, float zz, float posy, float escala, vector2* uv1, vector2* uv2, vector2* uv3, vector2* uv4, int frame, bool animation, bool isWater)
	{
		posx = xx;
		posz = zz;
		...
		if (isWater) {
			d3dContext->PSSetShaderResources(2, 1, &displacementMap);
		}
		...
		if (isWater) {
			D3DXMatrixRotationZ(&rotationMat, 90 * (D3DX_PI/180));
		}
		...
		d3dContext->UpdateSubresource(MovimientoWater, 0, 0, &Movimiento, 0, 0);
