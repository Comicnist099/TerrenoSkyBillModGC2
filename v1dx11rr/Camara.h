...
public:
	...
	Camara(D3DXVECTOR3 eye, D3DXVECTOR3 target, D3DXVECTOR3 up, int Ancho, int Alto)
	{
		...
		//la de proyeccion
		D3DXMatrixPerspectiveFovLH( &proyeccion, D3DX_PI/3.0, ancho / alto, 0.01f, 1000.0f );
	...
	//void SetPosition(float x, float y, float z) {
		//posCam = D3DXVECTOR3(x, y, z);
	//}


	D3DXMATRIX UpdateCam(float velZ, float velX,float arriaba, float izqder)
	{
		D3DXVECTOR4 tempo;
		...
		posCam += refFront * velZ / 10.0;
		posCam += refRight * velX / 10.0;
