...
public:
	float rotation; //cambio camara

	ModeloRR(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, float _posX, float _posZ, float collisionRange)
	{
		...
		posX = _posX;
		posZ = _posZ;
		this->MaxX = this->posX + collisionRange;
		this->MaxZ = this->posZ + collisionRange;
		this->MinX = this->posX - collisionRange;
		this->MinZ = this->posZ - collisionRange;
		this->RotY = 0;
	...
	bool GetdoOnce()
	{
		return elegibleForDoOnce;
	}
	void invalidateDoOnce()
	{
		elegibleForDoOnce = false;
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex)
	{
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &solCB);
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &valBCB);
	...
	void Update(float x, float z) //cambio camara
	{
		posX = x;
		posZ = z;
	}
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale)
	{
		if (isVisible == true)
		{
			...
			D3DXMATRIX scaleMat;
			D3DXMatrixScaling(&scaleMat, scale / 1.5, scale, scale / 1.3);
			...
			d3dContext->UpdateSubresource(solCB, 0, 0, &luzx, 0, 0);
			d3dContext->UpdateSubresource(valBCB, 0, 0, &valB, 0, 0);
			...
			d3dContext->VSSetConstantBuffers(5, 1, &solCB);
			d3dContext->VSSetConstantBuffers(6, 1, &valBCB);
