...
public:
	...
	bool LoadContent(WCHAR* diffuseTex, WCHAR* diffuseTex2)
	{
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), diffuseTex, 0, 0, &texturaDia, 0);
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), diffuseTex2, 0, 0, &texturaNoche, 0);
		...
		d3dResult = (*d3dDevice)->CreateBuffer(&constDesc, 0, &cambioCB);
	...
	void Render(D3DXVECTOR3 trans, float angle)
	{
		...
		(*d3dContext)->PSSetShaderResources(0, 1, &texturaDia);
		...
		(*d3dContext)->UpdateSubresource(cambioCB, 0, 0, &blending, 0, 0);
		(*d3dContext)->VSSetConstantBuffers(0, 1, &matrixBufferCB);
		(*d3dContext)->VSSetConstantBuffers(1, 1, &cambioCB);
