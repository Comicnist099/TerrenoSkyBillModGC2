...
public:
	TerrenoRR(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext)
	{
		...
		CargaParametros(L"Piedra.jpg", L"alturas.jpg", 100.0f);
	}
	...
	bool CargaParametros(WCHAR* diffuseTex, WCHAR* heightTex, float tile)
	{
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"pasto.jpg", 0, 0, &colorMap2, 0 );
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"arena.jpg", 0, 0, &colorMap3, 0 );
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"rgb.jpg", 0, 0, &blendMap, 0 );
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &oscurecerCB);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion)
	{
		...
		static float luzx = -800.0f;
		rotation += 0.01;
		if (luzx <= 800)
			luzx += .05;
		else luzx = -800;

		static float blending = 0;
		static float oscurecer = 1;
		static float gira = 0;
		gira += .0005;
		float cambio;
		cambio = 2 * sin(gira) - 1;
		...
		d3dContext->UpdateSubresource(oscurecerCB, 0, 0, &oscurecer, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(3, 1, &solCB);
		d3dContext->VSSetConstantBuffers(4, 1, &oscurecerCB);
